...
Imports System.Net
Imports System.IO
Public Class wsapplicationmastersearch
    Inherits System.Web.UI.Page
    Public MyConnection As SqlConnection
    '02/17/2021 BAN Created
    ...
    Sub Search(ByVal Sender As Object, ByVal e As GridViewDeleteEventArgs)
        ...
        s = s & "if (window.opener.document.title == 'WS Application Area Maintenance') {"
    ...
    Sub BindGrid()
        ...
        If getapplicationid.Value = "" And getapplicationname.Value = "" Then
            mySQL = "SELECT application_id, application_name, inactive FROM vw_ws_application_master"
            mySQL = mySQL & " WHERE inactive = "
            mySQL = mySQL & Chr(39) & "0" & Chr(39)
            mySQL = mySQL & " ORDER BY application_id"
        Else
            If getapplicationid.Value <> "" Then
                mySQL = "SELECT application_id, application_name, inactive FROM vw_ws_application_master WHERE application_id like "
                mySQL = mySQL & Chr(39) & "%" & getapplicationid.Value & "%" & Chr(39)
                'mySQL = mySQL & " AND inactive = "
                'mySQL = mySQL & chr(39) & "0" & chr(39)
                mySQL = mySQL & " ORDER BY application_id"
            ElseIf getapplicationname.value <> "" Then
                mySQL = "SELECT application_id, application_name, inactive FROM vw_ws_application_master WHERE application_name like "
                mySQL = mySQL & Chr(39) & "%" & getapplicationname.Value & "%" & Chr(39)
                'mySQL = mySQL & " AND inactive = "
                'mySQL = mySQL & chr(39) & "0" & chr(39)
                mySQL = mySQL & " ORDER BY application_name"
            End If
