...
Imports System.Net
Imports System.IO
Public Class wsgroupmastermaint
    Inherits System.Web.UI.Page
    Dim MyConnection As SqlConnection
    Sub AddGroup_Click(Sender As Object, E As EventArgs)
        ...
        If group_id.Text = "" Then
            ...
            AddMessage.InnerHtml = "Please enter the Group ID."
            group_id.Focus()
            Exit Sub
            'else
            'group_id.text = group_id.text.ToLower()
        Else
            If SQLValid_Entry(Trim(group_id.Text)) = False Then
                AddMessage.Style("color") = "red"
                AddMessage.InnerHtml = "You may not use a reserved word or special char in the Group ID."
                group_id.Focus()
                Exit Sub
            End If
        End If
        ...
        If group_name.Text = "" Then
            AddMessage.Style("color") = "red"
            AddMessage.InnerHtml = "Please enter the Group Name."
            group_name.Focus()
            Exit Sub
        Else
            If SQLValid_Entry(Trim(group_name.Text)) = False Then
                AddMessage.Style("color") = "red"
                AddMessage.InnerHtml = "You may not use a reserved word or special char in the Group Name."
                group_name.Focus()
                Exit Sub
            End If
        End If
        ...
        If SQLValid_Entry(Trim(group_id.Text)) = False Then
            AddMessage.Style("color") = "red"
            AddMessage.InnerHtml = "You may not use a reserved word or special char in a Group ID."
            group_id.Focus()
            Exit Sub
        End If
        ...
        If (Page.IsValid) Then
            ...
            InsertCmd = "INSERT INTO ws_group_master (group_id, group_name, inactive, creation_date, creation_user, modification_date, modification_user) "
            InsertCmd = InsertCmd & "VALUES (@groupid, @groupname, @inactive, @creationdate, @creationuser, @modificationdate, @modificationuser)"

            MyCommand = New SqlCommand(InsertCmd, MyConnection)

            MyCommand.Parameters.Add(New SqlParameter("@groupid", SqlDbType.VarChar, 20))
            MyCommand.Parameters("@groupid").Value = Trim(group_id.Text)

            MyCommand.Parameters.Add(New SqlParameter("@groupname", SqlDbType.VarChar, 100))
            MyCommand.Parameters("@groupname").Value = Trim(Server.HtmlEncode(group_name.Text))
            ...
            Try
                ...
                AddMessage.InnerHtml = group_id.Text.ToString() & "<b> Record Added</b><br>"
                group_id.Focus()
    ...
    Sub BindGrid(ByVal SortField As String)
        ...
        mySQL = "SELECT group_id, group_name, inactive, creation_date, "
        mySQL = mySQL & "creation_user, modification_date, modification_user "
        mySQL = mySQL & "FROM ws_group_master"
        If SortField <> "" Then
        ...
        DS = New DataSet()
        MyCommand.Fill(DS, "ws_group_master")

        MyGridView.DataSource = DS.Tables("ws_group_master").DefaultView
        MyGridView.DataBind()
    End Sub
    ...
    Sub gv_RowDataBound(ByVal Sender As Object, ByVal e As GridViewRowEventArgs)
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            If linkBtn.CommandName = "Delete" Then
                linkBtn.OnClientClick = "return confirm('group Records will be removed in Web Security. Are you sure you want to delete this entry?');"
    ...
    Sub gv_RowUpdating(ByVal Sender As Object, ByVal e As GridViewUpdateEventArgs)
        ...
        If blnGo Then
            ...
            strdgSQL = "UPDATE ws_group_master SET " &
    ...
    Sub gv_RowDeleting(ByVal Sender As Object, ByVal e As GridViewDeleteEventArgs)
        ...
        'If DeleteAllgroupInfo(Sender, e) = False Then ' Unable to delete User Area / User Area Group info
        '    Message.InnerHtml = "ERROR: Could not delete the User records."
        '    Message.Style("color") = "red"
        '    Exit Sub
        'End If

        strgroup_id = MyGridView.DataKeys(e.RowIndex).Value

        'Once done with initial authouring uncomment out
        If GroupUserExists(strgroup_id) = True Then
            Message.InnerHtml = "ERROR: Group User records with the Group ID exist. Can not delete the Group record. Inactivate."
            Message.Style("color") = "red"
            Exit Sub
        End If

        If GroupApplicationAreaExists(strgroup_id) = True Then
            Message.InnerHtml = "ERROR: Group Appliation records with the Group ID exist. Can not delete the Group record. Inactivate."
            Message.Style("color") = "red"
            Exit Sub
        End If

        Dim strdgSQL As String = "DELETE FROM ws_group_master WHERE group_id = " & Chr(39) & MyGridView.DataKeys(e.RowIndex).Value & Chr(39)
        ' Response.Write(strdgSQL)
    ...
    Function DeleteAllGroupInfo(ByVal Sender As Object, ByVal E As GridViewDeleteEventArgs) As Boolean
        'Dim MyCommand As SqlCommand
        'Dim mySQL As String

        DeleteAllGroupInfo = False

        ''Delete user area group data
        'mySQL = "DELETE FROM group_master WHERE group_id = "
    ...
    Sub gv_RowEditing(ByVal Sender As Object, ByVal E As GridViewEditEventArgs)
        ...
        MyGridView.Rows(E.NewEditIndex).FindControl("groupname").Focus()
    ...
    Sub gv_PreRender(ByVal Sender As Object, ByVal e As System.EventArgs)
        ...
        If MyGridView.EditIndex > -1 Then
            ...
            'group Name
            Dim GroupNameTextBox As TextBox = CType(row.FindControl("groupname"), TextBox)
            GroupNameTextBox.Width = Unit.Parse("200px")
            GroupNameTextBox.MaxLength = 40
    ...
    Function GroupUserExists(ByVal strgroup_id As String) As Boolean
        Dim DSum As DataSet
        Dim MyCommand As SqlDataAdapter
        Dim mySQL As String
        ...
        mySQL = "SELECT * FROM vw_ws_group_user WHERE group_id = "
        mySQL = mySQL & Chr(39) & strgroup_id & Chr(39)
        ...
        'fill dataset
        DSum = New DataSet()
        MyCommand.Fill(DSum, "vw_ws_group_user")
        mycount = DSum.Tables("vw_ws_group_user").Rows.Count
    ...
    Function GroupApplicationAreaExists(ByVal strgroup_id As String) As Boolean
        Dim DSam As DataSet
        Dim MyCommand As SqlDataAdapter
        Dim mySQL As String
        Dim mycount As Integer
        ...
        mySQL = "SELECT * FROM vw_ws_group_application_area WHERE group_id = "
        mySQL = mySQL & Chr(39) & strgroup_id & Chr(39)
        ...
        'fill dataset
        DSam = New DataSet()
        MyCommand.Fill(DSam, "vw_ws_group_application_area")
        mycount = DSam.Tables("vw_ws_group_application_area").Rows.Count
    ...
    Function RecordChangedSinceRead(ByVal strkey_id As String, ByVal beforemoddate As String) As Boolean
        ...
        myprSQL = "SELECT * FROM ws_group_master WHERE group_id = "
        ...
        'fill dataset
        DSpr = New DataSet()
        MyCommand.Fill(DSpr, "ws_group_master")
        mycount = DSpr.Tables("ws_group_master").Rows.Count

        'response.write("Window date = " & beforemoddate)
        recordchanged = True
        If mycount > 0 Then
            'response.write("DB Date = " & DSpr.Tables("groupmmaster").Rows(0)("modification_date"))		   

            'Only allow the change if someone has not changed the record inbetween
            If beforemoddate = DSpr.Tables("ws_group_master").Rows(0)("modification_date") Then
    ...
    Sub PrintData(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        s = s & Chr(34) & "wsgroupmastermaint.rpt" & Chr(34)
        s = s & ") </"
        s = s & "SCRIPT>"
