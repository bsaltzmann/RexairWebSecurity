...
Imports System.IO
Public Class rxcustomermastersearch
    ...
    Sub Search(Sender As Object, e As GridViewDeleteEventArgs)
        ...
        customerid = MyGridView.DataKeys(e.RowIndex).Value()

        s = "<SCRIPT language='javascript'>"
        s = s & "window.opener.document.all['" 'Reference to original window
        s = s & Request.QueryString("src") 'Reference to original window
        s = s & "'].value ='" + customerid + "';"
        s = s & "if (window.opener.document.title == 'RX View Customer Data') {"
    ...
    Sub BindGrid()
        ...
        If getcustomerid.Value = "" And getcustomername.Value = "" Then
            mySQL = "SELECT customer_number, customer_name, cust_deliv_seq, customer_status, state, customer_class, addressing_country, termination_date, last_purchase_date, "
            mySQL = mySQL & "responsible_to_number, responsible_to_name, rgd_number, rgd_name FROM vw_AS400_Customer_Master_All "
            mySQL = mySQL & "WHERE company_number = 'RX' AND cust_deliv_seq = '000' AND customer_number like "
            mySQL = mySQL & Chr(39) & "-1" & Chr(39)
            mySQL = mySQL & " ORDER BY customer_number"
        Else
            If getcustomerid.Value <> "" Then
                mySQL = "SELECT customer_number, customer_name, cust_deliv_seq, customer_status, state, customer_class, addressing_country, termination_date, last_purchase_date, "
                mySQL = mySQL & "responsible_to_number, responsible_to_name, rgd_number, rgd_name FROM vw_AS400_Customer_Master_All "
                mySQL = mySQL & "WHERE company_number = 'RX' AND cust_deliv_seq = '000' AND customer_number like "
                mySQL = mySQL & Chr(39) & "%" & getcustomerid.Value & "%" & Chr(39)
                mySQL = mySQL & " ORDER BY customer_number"
            ElseIf getcustomername.value <> "" Then
                mySQL = "SELECT customer_number, customer_name, cust_deliv_seq, customer_status, state, customer_class, addressing_country, termination_date, last_purchase_date, "
                mySQL = mySQL & "responsible_to_number, responsible_to_name, rgd_number, rgd_name FROM vw_AS400_Customer_Master_All "
                mySQL = mySQL & "WHERE company_number = 'RX' AND cust_deliv_seq = '000' AND customer_name like "
                mySQL = mySQL & Chr(39) & "%" & getcustomername.Value & "%" & Chr(39)
                'mySQL = mySQL & " AND inactive = "
                'mySQL = mySQL & chr(39) & "0" & chr(39)
                mySQL = mySQL & " ORDER BY customer_name"
            End If
    ...
    Sub Page_Load(Sender As Object, E As EventArgs)
        ...
        If Not (IsPostBack) Then
            CustSelect.SelectedIndex = 0 ' Customer Name
            getcustomerid.Disabled = True
            getcustomername.Disabled = False
            getcustomername.Focus()
    ...
    Sub DropChange_Click(Sender As Object, E As EventArgs)
        ...
        If CustSelect.SelectedIndex = 0 Then 'Customer Name
            getcustomerid.Disabled = True
            getcustomername.Disabled = False
            getcustomername.Focus()
        Else
            getcustomername.Disabled = True
            getcustomerid.Disabled = False
            getcustomerid.Focus()
        End If
